...
All scripts live at `github.com/codetot-web/runcloud-bash-scripts` (or whichever fork the team standardizes on) and install to `/usr/local/bin/` with a `ct-` prefix via `install-tools.sh`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/codetot-web/runcloud-bash-scripts/main/install-tools.sh | sudo bash
```
