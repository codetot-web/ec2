...
The five "Findings to consider patching" items above were each filed as a separate issue and all resolved in PR #7 (shipped in v0.1.0):

| Issue | Finding | Resolution |
|---|---|---|
| #2 | Pre-existing swap partition not detected | Fixed in #7 — `step_swap` now checks for any active swap |
| #3 | `step_install_wp_cli` re-downloads on every run | Fixed in #7 — skip when `wp` works |
| #4 | `step_rds_ca_bundle` always re-downloads | Fixed in #7 — skip when bundle parses |
| #5 | `step_summary` swap row truncated | Fixed in #7 — list every active swap with size |
| #6 | No reboot prompt after kernel upgrade | Fixed in #7 — warn via `/var/run/reboot-required` |
...
3. ✓ Patches landed in PR #7, released in v0.1.0.
4. ✓ Issue #1 closes when this plan merges.
